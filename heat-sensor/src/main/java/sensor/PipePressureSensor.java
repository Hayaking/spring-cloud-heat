...
@Data
@Slf4j
public class PipePressureSensor extends Thread {
    private final String sensorNamePrefix = "pressure";
    private final int type = ComponetType.PressureSensor.getType();
    private final BlockingQueue<HeatData> queue = Common.getQueue();
    private final List<Component> componentList = asList(
            Component.builder().lon( 113.59561 ).lat(34.752687 ).type( type ).sensorSuffix( 1 ).build()
    );

    private final List<Metric> metricList = asList(
            Metric.builder().name( pipeline_water_pressure.name() ).min( 50d ).max( 100d ).sensorId( sensorNamePrefix ).build(),
            Metric.builder().name( pipeline_water_pressure_increase.name() ).min( 50d ).max( 100d ).sensorId( sensorNamePrefix ).build(),
