...
@Data
@Slf4j
public class PipeFlowSensor extends Thread {
    private final String sensorNamePrefix = "flow";
    private final int type = ComponetType.FlowSensor.getType();
    private final BlockingQueue<HeatData> queue = Common.getQueue();
    private final List<Component> componentList = asList(
            Component.builder().lon( 113.594424).lat(34.752732 ).type( type ).sensorSuffix( 1 ).build()
    );

    private final List<Metric> metricList = asList(
            Metric.builder().name( pipeline_water_flow.name() ).min( 50d ).max( 100d ).sensorId( sensorNamePrefix ).build(),
            Metric.builder().name( pipeline_water_tassels.name() ).min( 50d ).max( 100d ).sensorId( sensorNamePrefix ).build(),
            Metric.builder().name( pipeline_water_level.name() ).min( 50d ).max( 100d ).sensorId( sensorNamePrefix ).build(),
